...
- 初めての制作するのでいろいろ調べながらの上製作時間に余裕がないため、最低限ゲームとしての体裁が整っていることを目標に制作しました。
  
## 制作内容について
- ダンジョンを探索しターン性コマンドバトルにする事は、ダンジョンゲームが好きなこともありすぐ決定しました。
- マップ・敵モンスター・アイテム・イベントは CSVでデータを作成し読み込む。
- 敵モンスター・アイテム・イベントのデータは、それぞれクラスに格納しディクショナリーに追加。
- いろいろ盛り込みたいことはありましたが、時間があれば追加ということで現状のものになりました。
- アイテムの説明を表示する予定でしたが、今回未実装。

## 移動時の仕様
- 移動時には入力した方向が移動可能であるかチェックし、可能ならば移動後に移動先のイベントをチェックしイベントがあれば発生処理。
...
- 毒状態はトラップイベントにて変化。
- ある間隔での移動毎に視界が狭くなっていきマップの見通しが悪くなる。松明を使用することで視界は回復する。
...
- ターン終了時にリミットが減り、バトル時でもリミットが０になるとゲームオーバー。
...
```
## マップの仕様
- マップのデータは、移動判定用の配列とイベント用の配列２つに格納。イベントは通路でしか起きない仕様なので、移動判定用配列の値は通行可(0)。
- プレイヤーの移動時、移動判定用の配列で移動可能か判別し、移動が可能ならばイベント用の配列の値をチェックして処理を分岐し、イベントがある場合ディクショナリーからデータを参照。
...
- 上記 E0～E9・GG以外のIDはイベント用配列にIDを格納。イベントがない地点の値は null。
